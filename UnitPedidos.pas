...
begin
    ...
    if key = 13 then begin
      if  InputQuery('Dados do Produto',[ 'Informe a Qtde','Infome o Valor Unitário'], Valores) then
        begin
    ...
    if key = 46 then begin
          if messagebox(0,'Você confirma excluir o Item  ?','Pergunta',mb_yesno+mb_iconquestion)=6 then
            begin
              ApagarItem(iditem);
            end
    end;
